...
@objc protocol XMCCameraFrontDelegate {
    func cameraSessionConfigurationDidComplete()
    func cameraSessionDidBegin()
    func cameraSessionDidStop()
}

class XMCCameraFront: NSObject {
    ...
    init(sender: AnyObject) {
        ...
        self.setObservers()
        if position != nil {
            self.initializeSession(position!)
            
        }
        else
        {
            self.initializeSession(AVCaptureDevicePosition.Front)
    ...
    func initializeSession(position: AVCaptureDevicePosition) {
